...
def get_webpage(symbol, company_name):
    try:
        ...
        book_value = float(driver.find_element_by_xpath('//*[@id="mainContent_updAddRatios"]/div[8]/p/span').get_attribute('innerHTML').replace(',',''))
        stock_cmp = float(driver.find_element_by_xpath('//*[@id="mainContent_clsprice"]/span/span').get_attribute('innerHTML').replace(',',''))
        mcap = float(driver.find_element_by_xpath('//*[@id="mainContent_updAddRatios"]/div[1]/p/span').get_attribute('innerHTML').replace(',',''))
        cash = float(driver.find_element_by_xpath('//*[@id="mainContent_ltrlCash"]/span').get_attribute('innerHTML').replace(',',''))
        debt = float(driver.find_element_by_xpath('//*[@id="mainContent_ltrlDebt"]/span').get_attribute('innerHTML').replace(',',''))
        roe = float(driver.find_element_by_xpath('//*[@id="mainContent_updAddRatios"]/div[14]/p/span').get_attribute('innerHTML').replace(',',''))
        eps = float(driver.find_element_by_xpath('//*[@id="mainContent_updAddRatios"]/div[12]/p/span').get_attribute('innerHTML').replace(',',''))
...
def read_nse_companies():
    df = pd.read_excel('NSE_MAR22.xlsx',index_col=0)
...
if __name__ == "__main__":
    ...
    csv_file = open('attractive_stonks_mar2022.csv', 'w')
